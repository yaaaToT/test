//
// Created by torosamy on 24-11-19.
//
#include "../../../../include/pod/multiton/package/DataPacket.hpp"
unsigned char rdata[255];
const uint8_t CRC8_INIT     = 0xff;
const uint8_t CRC8_TAB[256] = {
    ,
};

DataPacket::DataPacket(const int& id) : mGunId(id) {
}


unsigned char DataPacket::getHead() const{
    return mHead;
}

unsigned char DataPacket::getTail()  const{
    return mTail;
}

int DataPacket::getId() const{
    return this->mGunId;
}


uint8_t DataPacket::crc8Check(uint8_t *addr, int len) {
    uint8_t data;
    uint8_t crc = CRC8_INIT; // 初始值
    while (len--) {
        data = *addr++;
        crc = crc ^ data;    // 与crc初始值异或
        crc = CRC8_TAB[crc]; // 替换下面for循环
    }
    return crc; // 返回最终校验值
}

void DataPacket::sendData(int& fd) {
    std::cout<<"send data"<<std::endl;
}
void DataPacket::receiveData(int& fd) {
    std::cout<<"receive data"<<std::endl;
}
